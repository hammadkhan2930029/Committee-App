...
export const UserCommitteeDetails = ({ route }) => {
    ...
    const memberCountMap = roundList.reduce((acc, item) => {
        const id = String(item?.committee_member_id ?? '').trim();
        if (!id) return acc;
        acc[id] = (acc[id] || 0) + 1;
        return acc;
    }, {});
    ...
    const isPastOrCurrentMonth = (roundMonth) => {
        ...
        const todayDate = now.getDate(); // Aaj ki date (e.g., 8)
        const apiDate = details.start_date
        const activationDate = apiDate ? parseInt(apiDate.split('')[0]) : 1

        console.log('activationDate :', activationDate)


        // फर्ज करें आपकी कमेटी हर महीने की 8 तारीख को शुरू होती है
        // const activationDate = 8;
        ...
        // 1. Agar saal purana hai
        if (roundYear < currentYear) {
            return true;
        }

        // 2. Agar saal current hai
        if (roundYear === currentYear) {
            // Pichly mahine saare enable honge
            if (roundMonthIndex < currentMonthIndex) {
                return true;
            }
            // Current mahine mein tabhi enable hoga jab activation date (8) aa chuki ho
            if (roundMonthIndex === currentMonthIndex) {
                return todayDate >= activationDate;
            }
        }
    ...
    //   };
    //-------------------------------------------------------------------------
    const memberId = String(data?.committee_member_id ?? '').trim();
    const memberCount = memberCountMap[memberId] || 1;
    const totalRounds = filterRound.length
    const perRoundAmount = Number(details?.amount_per_member || 0);
                                                    ...
                                                    <CustomButton
                                                        ...
                                                        onPress={() =>
                                                            navigation.navigate('UploadSlip', {
                                                                singleRoundAmount: perRoundAmount,
                                                                amount: perRoundAmount * totalRounds,
                                                                totalRounds,
                                                                data,
                                                            })
